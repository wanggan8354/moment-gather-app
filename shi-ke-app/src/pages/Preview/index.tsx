...
export default function Preview() {
  ...
  const work = currentWork?.id === id ? currentWork : works.find((w) => w.id === id) || works[0];
  ...
  const totalDurationSec = work ? parseInt(work.duration.split(':')[0]) * 60 + parseInt(work.duration.split(':')[1]) : 154;

  const socialPlatforms = [
    { name: '微信', color: '#07C160', icon: WeChatIcon },
    { name: '朋友圈', color: '#07C160', icon: MomentsIcon },
    { name: '抖音', color: 'linear-gradient(135deg, #1a1a2e, #16213e)', icon: DouyinIcon },
    { name: '微博', color: '#E6162D', icon: WeiboIcon },
  ];

  return (
    <MobileShell>
      <div className="relative" style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
        <nav
        ...
        >
          ...
          <h1 className="text-sm font-semibold tracking-wide" style={{ color: 'var(--color-text-primary)' }}>
            短片预览
          </h1>
          <button className="transition-colors active:opacity-70" style={{ color: 'var(--color-text-secondary)' }}>
            <MoreHorizontal size={20} />
          </button>
        </nav>

        <section className="relative overflow-hidden" style={{ background: '#1a1008' }}>
          <div
          ...
          <div className="relative mx-7" style={{ aspectRatio: '16/9' }}>
            <img
              ...
              style={{ filter: isPlaying ? 'bright(1) contrast(1)' : 'brightness(0.85) contrast(1.05) saturate(1.1)' }}
            />
            ...
            >
              {!isPlaying && (
                <>
                  ...
                  >
                    <Play size={28} style={{ color: 'var(--color-primary-dark)', marginLeft: 4 }} fill="var(--color-primary-dark)" />
                  </div>
                </>
              )}
            </button>
          ...
          <div className="px-8 py-3" style={{ background: 'linear-gradient(to top, rgba(26,16,8,0.9), rgba(26,16,8,0.4))' }}>
            <input
              type="range"
              min={0}
              max={100}
        ...
        <section className="px-5 pt-5 pb-4">
          ...
          <p className="mt-2 text-sm leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
            AI 从 {work?.photoCount || 847} 张照片中精选 {work?.selectedMoments || 42} 个瞬间
          </p>
        ...
        <div className="mx-5 h-px" style={{ background: 'var(--color-border-light)' }} />
        ...
        <section className="px-5 py-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <button className="flex flex-col items-center gap-2 active:scale-90 transition-transform">
              ...
              >
                <MoreHorizontal size={22} style={{ color: 'var(--color-text-tertiary)' }} />
              </div>
              <span className="text-xs" style={{ color: 'var(--color-text-tertiary)' }}>
                更多
              </span>
        ...
        <section className="px-5 py-4 pb-8">
          ...
          <div className="rounded-xl overflow-hidden border" style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}>
            {[
              { icon: Download, label: '保存到相册', iconBg: 'var(--color-primary-bg)', iconColor: 'var(--color-primary)' },
              { icon: Disc, label: '制作 DVD', iconBg: 'var(--color-primary-bg)', iconColor: 'var(--color-primary)' },
              { icon: Mail, label: '制作明信片', iconBg: 'var(--color-primary-bg)', iconColor: 'var(--color-primary)' },
              { icon: Trash2, label: '删除', iconBg: '#FEF2F2', iconColor: 'var(--state-error)' },
              ...
              <button
                ...
                style={{
                  borderBottom: idx < 3 ? '1px solid var(--color-border-light)' : 'none',
                }}
              >
                <div
                ...
                <span className="text-sm font-medium flex-1" style={{ color: idx === 3 ? 'var(--state-error)' : 'var(--color-text-primary)' }}>
                  {label}
                </span>
                <ChevronRight size={16} style={{ color: 'var(--color-text-tertiary)', opacity: idx === 3 ? 0.5 : 1 }} />
              </button>
            ))}
          </div>
        </section>
...
function DouyinIcon() {
  return (
    <svg width={22} height={22} viewBox="0 0 24 24" fill="none">
      <path d="M16.6 5.82C15.56 4.94 14.86 3.71 14.74 2.32c-.01-.14-.01-.28-.01-.42V1.5h-3.68v14.28c0 1.42-1.14 2.58-2.56 2.58-1.42 0-2.58-1.16-2.58-2.58s1.16-2.58 2.58-2.58c.28 0 .54.04.78.12V9.48c-.26-.04-.52-.06-.78-.06-3.82 0-6.92 3.1-6.92 6.92s3.1 6.92 6.92 6.92 6.92-3.1 6.92-6.92V9.5c1.18.88 2.64 1.4 4.22 1.4V7.22c-1.52 0-2.9-.56-3.96-1.4z" fill="#FE2C55" />
    </svg>
  );
}
...
function WeiboIcon() {
  return (
    <svg width={22} height={22} viewBox="0 0 24 24" fill="white">
      ...
      <path d="M19.47 5.069c.643-.716 1.107-1.586 1.107-2.517 0-1.537-1.248-2.552-2.776-2.552-1.528 0-2.776 1.015-2.776 2.552 0 .931.464 1.801 1.107 2.517.643.716 1.669 1.248 1.669 1.248s1.026-.532 1.669-1.248z" fill="#FF9933" />
    </svg>
  );
}
